...
class Options_Framework_Media_Uploader {
	...
	static function optionsframework_uploader( $_id, $_value, $_desc = '', $_name = '' ) {
		...
		if ( function_exists( 'wp_enqueue_media' ) ) {
			if ( ( $value == '' ) ) {
				$output .= '<input id="upload-' . $id . '" class="upload-button button" type="button" value="' . __( '添加', 'theme-textdomain' ) . '" />' . "\n";
			} else {
		...
		if ( $value != '' ) {
			...
			$image = preg_match( '/(^.*\.jpg|jpeg|svg|png|gif|ico*)/i', $value );
			if ( $image ) {
